...
function App() {
  ...
  useEffect(() => {
    const savedUser = sessionStorage.getItem('hc_admin_user');
    const token = sessionStorage.getItem('hc_admin_token');
    if (savedUser && token) {
            ...
            <div className="h-full flex flex-col relative">
              ...
              <div className={`p-4 border-t border-slate-100 transition-all duration-300 ${isSidebarOpen ? '' : 'px-2'}`}>
                <button 
                  ...
                  className={`w-full flex items-center text-slate-500 hover:bg-rose-50 hover:text-rose-600 rounded-xl transition-all group ${isSidebarOpen ? 'space-x-3 px-4 py-3' : 'justify-center py-4'}`}
                >
              ...
              <div className="flex items-center space-x-4">
                ...
                <Link to="/profile" className="flex items-center space-x-3 cursor-pointer group">
                  <div className="text-right hidden sm:block">
                    <p className="text-sm font-bold text-slate-700 leading-none">{user?.username || 'Admin User'}</p>
                    <p className="text-[10px] text-slate-400 font-bold uppercase mt-1">{user?.role || 'Super Admin'}</p>
                  </div>
